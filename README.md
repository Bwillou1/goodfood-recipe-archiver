...
> **Récupère tes vraies fiches recettes Goodfood et les compile en un livre PDF complet en moins de 10 secondes.**
...
- 🚀 **Temps d'exécution total : ~8 à 10 secondes** pour 3 fiches officielles 2 pages HD.
...
- 🛡️ **Bootstrap Sandbox Anti-OOM** : `scripts/bootstrap.sh` installe uniquement les bibliothèques système Chromium nécessaires (sans polices CJK géantes).
...
- **Filtrage immédiat des traceurs** : Neutralisation silencieuse de New Relic, TikTok, Snapchat, LinkedIn, Google Analytics, Datadog, DoubleClick, Cloudflare RUM.
...
python run.py --out "mon_livre.pdf"       # Spécifier le chemin de sortie du PDF
...
├── Dockerfile              # 🐳 Sandbox conteneurisée sécurisée
├── scripts/
│   └── bootstrap.sh        # 🔧 Initialisation automatique sans OOM
...
│   ├── ocr_receipt.py      # Extraction intelligente des noms de plats
│   ├── finder.py           # Découverte et cache de l'historique officiel
│   ├── pdf_builder.py      # Rendu PDF asynchrone et parallèle
│   └── assembler.py        # Fusion finale du livre de recettes avec métadonnées
└── data/
